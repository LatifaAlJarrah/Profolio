...
//     // Map templateType to default images
...
//       // Use media URL if available, otherwise pick image based on templateType
//       thumbnail:
...
async function unpublishTemplate(templateId: string, token: string) {
  try {
    const response = await fetch(
      `http://localhost:3001/api/templates/${templateId}/unpublish`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export default function UserTemplates({
  ...
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [userInfo, setUserInfo] = useState<UserInfo>({ user: null, token: null });
  ...
  useEffect(() => {
    const loadUserTemplates = async () => {
      try {
        ...
        const templatesData = await fetchUserTemplates(info.user.id, info.token || undefined);
        setTemplates(templatesData);
        console.log("Templates updated:", templatesData);
      } catch (err) {
        const errorMessage =
  ...
  const handlePublishToggle = async (template: UserTemplate) => {
    if (!userInfo.user?.name || !userInfo.token) {
      alert("You must be logged in with a valid username to publish templates");
      return;
    }
    ...
    try {
      if (template.isPublic && template.publishUrl) {
        const result = await unpublishTemplate(template.id, userInfo.token);
        setTemplates((prev) =>
          prev.map((t) =>
            t.id === template.id
              ? { ...t, isPublic: false, publishUrl: "" }
              : t
          )
        );
        ...
        setTemplates((prev) =>
          prev.map((t) =>
            t.id === template.id
              ? { ...t, isPublic: true, publishUrl: result.publishUrl }
              : t
          )
        );
  ...
  const copyPublishUrl = (publishUrl: string) => {
    const fullUrl = `http://localhost:3001/api/templates/public/${publishUrl}`;
              ...
              <div className="mt-3 px-2">
                ...
                <div className="mt-3 flex flex-col gap-2">
                  <button
                    ...
                    disabled={publishingIds.has(template.id)}
                    className={`px-3 py-1 text-xs rounded-full font-medium transition-colors ${
                      template.isPublic
                        ? "bg-red-100 text-red-700 hover:bg-red-200"
                        : "bg-green-100 text-green-700 hover:bg-green-200"
                    } ${
                      publishingIds.has(template.id)
                  ...
                  {template.publishUrl && (
                    <div className="flex items-center gap-2 text-black">
                      <input
                        type="text"
                        value={`http://localhost:3001/api/templates/public/${template.publishUrl}`}
                        readOnly
